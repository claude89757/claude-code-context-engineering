...
### Context Engineering 101 Skill (Interactive Learning)
- **7 Patterns** — Layered system prompt, extended thinking, reasoning effort, context management, tool lazy loading, dynamic injection, history replay / 7 大上下文工程模式
- **Ultrathink Deep Dive** — Thinking type, budget tokens, effort levels, and how they interact / Ultrathink 深度解析
- **Hands-on Analysis** — Ready-to-use jq commands for inspecting real trace data / 可直接使用的 jq 命令分析真实 trace 数据
- **Version Analysis** — Scripts to analyze and compare Claude Code across versions / 跨版本分析与对比脚本
...
Interactive learning skills for Claude Code, usable directly in Claude Code via `/context-engineering-101`.

交互式学习技能，可在 Claude Code 中直接通过 `/context-engineering-101` 调用。

### context-engineering-101

A structured skill for learning Claude Code's context engineering patterns, with focus on **ultrathink (extended thinking)**.

一个结构化的学习技能，用于深入理解 Claude Code 的上下文工程模式，重点关注 **ultrathink（扩展思维）**。

**Covers / 覆盖内容:**
- 7 context engineering patterns (layered system prompt, extended thinking, reasoning effort, context management, tool lazy loading, dynamic injection, history replay) / 7 大上下文工程模式
- Ultrathink deep dive (trigger methods, thinking/effort/budget configuration) / Ultrathink 深度解析
- Version analysis & cross-version comparison scripts / 版本分析与跨版本对比脚本
- Prerequisite auto-installer & troubleshooting guide / 前置依赖自动安装与故障排查
...
```
├── skills/                  # Claude Code interactive learning skills
│   └── context-engineering-101/
│       ├── SKILL.md         # Main skill (7 patterns + ultrathink deep dive)
│       ├── scripts/         # Prerequisites, version analysis, comparison
...
│   └── docker-compose.yml
└── .claude-trace/           # Trace data and analysis reports
```
